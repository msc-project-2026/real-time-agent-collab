...
async function validateUrl(url) {
  ...
  // Resolve all address families with a timeout — an IPv6-only private address
  // would bypass an IPv4-only check, and a hung DNS lookup would stall the bot.
  const addresses = await Promise.race([
    dns.lookup(hostname, { all: true }),
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error(`DNS lookup for ${hostname} timed out`)), 3000)
    ),
  ]);
