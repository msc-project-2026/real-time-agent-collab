...
// Resolve the GitHub App private key. Prefer a mounted PEM file
// (GITHUB_APP_PRIVATE_KEY_FILE) so the multiline key need not live in .env;
// fall back to an inline value, unescaping literal \n sequences.
...
async function writeFile(owner, repo, path, content, commitMessage) {
  const token = await getInstallationToken(owner, repo);
