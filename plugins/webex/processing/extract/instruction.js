...
- For \`assigned\`: can you place this on a specific space member, based on who's speaking or mentioned in the thread? If so, set \`assigned\` to their \`id\` from the space members list above — never their name, and never a guessed id for someone not in the list (fall back to their name as free text instead). If not, default to \`assigned: "agent"\` and set \`confidence\` per the criteria below.
