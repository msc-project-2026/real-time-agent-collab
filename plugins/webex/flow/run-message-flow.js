...
async function runMessageFlow({
  ...
  if (flowId) {
    await appendJobLogEntry({
      ...
      flowId,
      step: 'gate',
      runId: tagResult?.runId ?? null,
      sessionKey: tagResult?.sessionKey ?? null,
      sessionId: tagResult?.sessionId ?? null,
      usage: tagResult?.usage ?? null,
  ...
  log?.info?.(
    `${LOG_PREFIX} dispatch decision ${JSON.stringify({
      ...
      gateRunId: tagResult?.runId ?? null,
  ...
  if (flowId) {
    await appendJobLogEntry({
      ...
      sessionId: extractResult?.sessionId ?? null,
      usage: extractResult?.usage ?? null,
    ...
    await appendJobLogEntry({
      ...
      sessionId: summarizeResult?.sessionId ?? null,
      usage: summarizeResult?.usage ?? null,
  ...
  if (flowId) {
    await appendJobLogEntry({
      ...
      sessionId: respondResult?.sessionId ?? null,
      usage: respondResult?.usage ?? null,
