    ...
    try {
      ...
      if (envelope && pluginRuntime) {
        ...
        const ctxPayload = {
          ...
          ChatType:
            envelope.metadata.roomType === 'direct' ? 'direct' : 'group',
          SenderName:
...
exports.webexPlugin = {
  ...
  config: {
    ...
    resolveAccount: (cfg, accountId) =>
      resolveWebexAccount({ cfg: cfg, accountId }),
    ...
    resolveAllowFrom: ({ cfg }) =>
      (cfg.channels?.webex?.allowFrom ?? []).map(String),
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // Keep channel alive until the gateway stops it.
      // Forked from @jimiford/webex v0.1.3: startAccount now must return a
      // long-running promise rather than a cleanup function (OpenClaw API change).
      try {
        await new Promise(() => {}); // never resolves — channel stays alive
      } finally {
