...
// Category: Routing LLM boundary.
// These tests verify compatible-provider discovery, deterministic request parameters, missing-provider behavior, and upstream error propagation.
describe('routing LLM boundary', () => {
  test('skips routing safely when no compatible provider exists', async (t) => {
    const fetchMock = t.mock.method(globalThis, 'fetch', async () => {
    ...
    const response = await callRoutingLlm({
      instruction: 'route this',
      pluginRuntime: { config: { current: () => ({ models: { providers: {} } }) } },
      log,
    });
  ...
  test('calls the configured provider with the fixed routing model', async (t) => {
    const fetchMock = t.mock.method(globalThis, 'fetch', async () => ({
      ok: true,
      json: async () => ({ choices: [{ message: { content: '{"routes":[]}' } }] }),
    }));
    const { callRoutingLlm } = require('./routing/direct-llm');
      ...
      pluginRuntime: {
        config: {
          current: () => ({
            models: {
              providers: {
                incompatible: { api: 'other', baseUrl: 'https://ignored', apiKey: 'x' },
                selected: {
  ...
  test('includes status and a bounded body snippet in provider failures', async (t) => {
  ...
  test('returns null when a compatible provider produces no message content', async (t) => {
    t.mock.method(globalThis, 'fetch', async () => ({
      ok: true,
      json: async () => ({ choices: [] }),
    }));
