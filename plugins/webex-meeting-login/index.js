...
Use only the browser tool in the sandboxed Brave browser. Do not use exec, web search, web fetch, messaging, or any other tool. Do not visit any URL other than ${WEBEX_SIGN_IN_URL}.
...
If the browser is already signed in, leave the existing signed-in session untouched and report success. If Webex displays MFA, CAPTCHA, a security challenge, or a login error, stop immediately and report only the status; do not attempt any alternate flow. Do not join, schedule, or interact with meetings. Never include the email address or password in your final response.`;
}
...
function register(api) {
  api.on('gateway_start', async () => {
    try {
      ...
      await dispatchLogin(api.runtime, buildLoginPrompt(email, password));
      api.logger?.info?.('[webex-meeting-login] startup login task completed');
    } catch (err) {
      api.logger?.error?.(`[webex-meeting-login] startup login task failed: ${err?.message ?? err}`);
    }
  }, { timeoutMs: 120000 });
}
