...
// for the `meetings` resource. Mirrors plugins/webex/token.js's pattern, scoped to this
// plugin's own webhookUrl and its own single `started` webhook.
//
// A bot token cannot read the meetings resource — this requires a licensed user's OAuth
// token (accessToken/refreshToken), same constraint as channels.webex.accessToken.
'use strict';
...
const WEBHOOKS = [
  { name: 'OpenClaw Meeting Started Handler', resource: 'meetings', event: 'started' },
];

// Deregisters any existing webhooks pointing at cfg.webhookUrl then recreates them.
