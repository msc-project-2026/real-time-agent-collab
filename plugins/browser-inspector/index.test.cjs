...
async function executeDetails(tool, params) {
...
test('registers the browser inspection tools and forwards their option shapes', async (t) => {
  ...
  assert.ok([...tools.values()].every((tool) => typeof tool.execute === 'function'));
  assert.deepEqual(await executeDetails(tools.get('inspect_webpage'), { url: 'https://example.test', check_links: true }), { inspected: true });
  assert.deepEqual(await executeDetails(tools.get('inspect_element'), { url: 'https://example.test', selector: '#app' }), { found: true });
  assert.deepEqual(await executeDetails(tools.get('get_page_structure'), { url: 'https://example.test' }), { headings: [] });
  ...
  assert.deepEqual(await executeDetails(tools.get('log_browser_issue'), { room_id: 'room', title: 'Broken link', severity: 'high', description: '404' }), { ok: true, id: 'ISSUE-7' });
  assert.deepEqual(await executeDetails(tools.get('read_source_file'), { room_id: 'room', file_path: 'src/App.js' }), { found: true, file_path: 'src/App.js', content: 'const current = true;' });
  assert.deepEqual(await executeDetails(tools.get('suggest_fix'), { room_id: 'room', file_path: 'src/App.js', proposed_content: 'fixed', explanation: 'Corrects the selector.' }), { ok: true, id: 'FIX-9', filePath: 'src/App.js', explanation: 'Corrects the selector.' });
  assert.deepEqual(await executeDetails(tools.get('commit_fix'), { room_id: 'room', suggestion_id: 'FIX-9' }), { ok: true, sha: 'abc123' });
  assert.deepEqual(await executeDetails(tools.get('reject_fix'), { room_id: 'room', suggestion_id: 'FIX-9' }), { ok: true, id: 'FIX-9' });
});
...
test('screenshot upload requires a token and surfaces a rejected Webex response', async (t) => {
  ...
  assert.deepEqual(await executeDetails(screenshot, { url: 'https://example.test', room_id: 'room' }), { ok: false, error: 'WEBEX_BOT_TOKEN not set — cannot post screenshot' });

  process.env.WEBEX_BOT_TOKEN = 'token';
  global.fetch = async () => ({ ok: false, status: 413, text: async () => 'payload too large' });
  assert.deepEqual(await executeDetails(screenshot, { url: 'https://example.test', room_id: 'room', full_page: true, viewport_width: 640, viewport_height: 480 }), { ok: false, error: 'Webex upload failed: 413 payload too large' });
  assert.deepEqual(browserCalls, [
...
test('storage-backed tools report disabled integrations and handler errors clearly', async (t) => {
  ...
  assert.deepEqual(await executeDetails(tools.get('log_browser_issue'), {}), { ok: false, error: 'collab-cache module not available — issues.md logging is disabled' });
  assert.deepEqual(await executeDetails(tools.get('read_source_file'), { room_id: 'room', file_path: 'missing.js' }), { found: false, file_path: 'missing.js' });
  assert.deepEqual(await executeDetails(tools.get('suggest_fix'), {}), { ok: false, error: 'collab-cache module not available, fix suggestions are disabled' });
  assert.deepEqual(await executeDetails(tools.get('commit_fix'), { room_id: 'room', suggestion_id: 'FIX-404' }), { ok: false, error: 'not approved' });
  assert.deepEqual(await executeDetails(tools.get('reject_fix'), { room_id: 'room', suggestion_id: 'FIX-404' }), { ok: false, error: 'missing suggestion' });
});
