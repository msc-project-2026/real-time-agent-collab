...
**Addition (config card consolidation):** alongside `senderId`, a `senderName` is resolved once and stored at write time, in `storage/threads-store.js`'s `formatMessageForThreadWindow` — the single point every downstream prompt-builder (gate/extract/summarize/respond) reads from, so none of them need their own lookup. Resolution order: an already-set `message.senderName` → the space's cached member mapping (`config/store.js`'s `active.json`, keyed by email, refreshed from Webex's own membership roster whenever the config card is opened — see §6) → raw `personEmail` → `personId` → `null`.

**Addition (space-members-in-prompt):** `config/members.js`'s `getSpaceMembers({ spaceId })` (id/name pairs, plus the always-present `AGENT_ASSIGNEE`) is now injected into two of the four steps, not baked into `agent-identity/AGENTS.md` (§8d) since it's per-space data, not a static identity fact. `extract/instruction.js` gets a "Space members" section so `assigned` (§7c) can resolve to a real member id instead of guessed free text — the model is told to match by id from that list when a task is clearly assigned to someone, falling back to free text only when the person genuinely isn't in it. `respond/instruction.js` folds the same list into its existing "Known facts" section (alongside `spaceId`/`boardUrl`) so replies can address people by their real name.
...
- Real replies (`respond`, via `channel.js`), config/task cards, task-notify's ack, and the config card's own submission confirmation/error messages all record by default (`recordToThread: true`) — genuine content worth the model remembering later.
...
**Correction (manual testing):** the config submission confirmation (`config/handle-submission.js`) was a real, silent gap for a while — it called the bare `sendWebexMessage` directly instead of `sendOutboundMessage`, so neither the "🟩 Configuration saved" nor the "🟥 not saved" message was ever recorded. A later question like "did you save my config?" would have found no trace of that exchange in the window. Fixed by switching both sends to `sendOutboundMessage` and threading `botId` two layers further than it previously reached (`inbound/attachment-actions.js` had it in its own `ctx` all along but never forwarded it).
...
### 3b. A reply target the bot can't see: Webex's mention-only visibility restriction

**Addition (manual testing, post-deploy):** Webex bots can only see messages that explicitly `@`-mention them — a real platform restriction, not a bug, and exactly why this plugin has a separate OAuth-based "message observer" identity (`channel.js`) with full visibility into every message for classification purposes. That identity never sends anything, though — every outbound send (config card, `respond`, task-notify, the thinking placeholder) goes through the bot's own token (`resolvedAccount.config.token`, `channel.js`'s `sendText`/`sendMedia`), since a message must come from the bot's own identity to actually appear as the bot speaking.

This creates a real gap: when the gate judges a message addressed via its own semantic inference alone (`isBotMentioned: false`, `isBotAddressed: true` — no literal mention, e.g. most `configRequest` phrasing), the bot's own token has never seen that message and has no visibility into it. Using it — or an existing thread's root, which may equally be invisible — as `parentId` gets rejected by Webex: `"Parent activity ID not found or invalid"` for a plain reply, or `"Cannot reply to a reply"` for the config card specifically (§6), since it's always sent as a reply to begin with.

Fixed once, centrally, rather than caught reactively per sender: `send.js`'s `resolveReplyThreadId({ spaceId, explicitRoot, threadKey, message, isBotMentioned })` — async, since the existing-thread branch below reads storage — and every sender (`config/card.js`, `respond/dispatch.js`, `processing/task-notify.js`, `processing/thinking-ack.js`) computes its reply target through it instead of its own inline ternary. A `null` result posts as an ordinary top-level message in the main space (`send.js`'s `buildMsgBody` only sets `parentId` when truthy) — a safe, correct fallback, not a threading regression. Deciding this up front, before ever building or sending anything, also keeps the config card's own `Action.Submit` data (which embeds the *same* `replyThreadId`, round-tripped back on submission — §6) consistent with what the initial card send actually used, with no reactive retry-and-rebuild needed.

**Correction (manual testing, second pass):** two distinct mention facts matter here, not one, and the first version of this fix conflated them. For the main space, the *triggering* message is the candidate reply root, so its own (deterministic) `isBotMentioned` is exactly the right check. But for an existing thread, the candidate root is a *different* message — whichever one started that thread — and being mentioned in a reply several messages deep into it says nothing about whether the bot's token can also see the *root*. Checking the triggering message's mention status for this branch was wrong; the fix reads the root's own mention status instead, via a `rootBotIsMentioned` flag stamped onto the thread record once, at creation — `storage/threads-store.js`'s root-backfill step (§2's root-seeding) computes it from the backfilled root message's own `mentionedPeople` (fetched via the OAuth identity, so it's accurate regardless of the bot's own restricted visibility) the moment a new non-main thread is first discovered, and it persists unchanged across every later append to that same thread. `resolveReplyThreadId` reads it with one `getThread` call — a local storage read, not a Webex API call — rather than scanning the window or re-fetching. An unconfirmed root (backfill fetch failed, or never attempted) defaults to `false` — treated the same as "not mentioned," since the safe fallback either way is posting bare in main, not guessing.
...
- `shouldProcess` → flush thread's pending slice, advance the flow and spawn `extract`/`summarize` (§7) — task/summary value doesn't depend on being spoken to, so these run whenever the slice is ready OR the bot was addressed, independent of each other. This independence is intentionally preserved even when `configRequest` is also true — a message can legitimately both ask to see the config and contain a task-worthy point.
- `shouldRespond` → spawn `respond` (§7), **sequenced after `extract`/`summarize`/`task-notify` settle** (see §5a below), not alongside them. **Correction (response-policy + extraction-calibration revision):** `respond` originally spawned on the same `shouldProcess` condition, with a prompt-level instruction to stay silent when only `sliceReady` (not addressed) was true. Manual testing showed this wasn't reliable — `respond` narrated an already-extracted, human-assigned task unprompted. Replaced with a controller-level gate: `respond` is only spawned at all when `shouldRespond` is true, guaranteeing silence rather than prompting for it. **Correction (manual testing, post-deploy):** asking to see the config is itself a direct question to the bot, so it also gets judged `isAddressed` — `shouldRespond` and `configRequest` firing together used to send the config card, a "thinking..." placeholder, *and* a redundant conversational reply for the same message. `respond` (and the thinking-ack that precedes it) is now also suppressed whenever `configRequest` is true, on the same "the deterministic path already covered this message" principle §5a establishes for task-notify below — the config card already is this message's reply.
...
**Correction (config card consolidation):** `CONFIDENCE_AUTO_APPROVE_THRESHOLD` (0.7) is now only the fallback default, not a fixed constant. Each space can set its own `proactivityThreshold` via the config card's "Agent proactivity" field (`Conservative` 0.85 / `Balanced` 0.7 / `Proactive` 0.5 — a `Input.ChoiceSet`, since Adaptive Cards have no native slider) — `processing/extract/tool.js`'s auto-approval check reads `readActiveConfig({ spaceId })?.config?.proactivityThreshold`, falling back to the constant when a space has never been configured. `tasks-store.js` still owns and exports the constant as the documented default; `config/proactivity.js` re-exports it rather than redefining it, so there's one source of truth.
...
**Correction (dedicated agent identity):** in production, every step (gate/extract/summarize/respond) currently shares a single identity, `collab-agent`, resolved via `runtime.js`'s `getCollabAgentId()` (`plugins.entries.webex.config.collabAgentId`, default `"main"` if unset) — not yet the one-identity-per-step split this section originally sketched. Before this, `collabAgentId` was never actually set on the deployed gateway, so every spawn silently defaulted to `"main"` — the host's own general-purpose agent — meaning whatever `main`'s own `AGENTS.md`/`SOUL.md` says (generic dev-assistant framing, unrelated project instructions) was injected as system-level content into every plugin spawn. `collab-agent` is registered in the host's `agents.list[]` with `agentDir: "plugins/webex/agent-identity"` (not `default: true`), so its `AGENTS.md` ships and versions with this plugin's own repo rather than as a manual host-side file. No explicit `workspace` override — the automatic `workspace-collab-agent` fallback is sufficient since every spawn here is tool-driven, not file-based. Splitting into per-step identities (`collab-gate`, `collab-extract`, etc.) remains a future, independent step — it only requires adding more `agents.list[]` entries and reading a per-step config key instead of one shared `collabAgentId`.
...
- The board's own **manual** delegate-button write-back — a human delegating an already-approved task through the board UI is still local-only state (`board/src/App.jsx`), not persisted. Only the automatic, confidence-driven path (§7c) is real: `tasks-store.js`'s `delegation` field and the auto-approval override in `write_task` are implemented and write through. Manual write-back is deferred to the larger board write-back update.
- **Correction (config card consolidation):** the space-member list (`config/members.js`) is no longer dummy data — it reads a real, Webex-refreshed cache (§3, §6), and is now threaded into both `extract` and `respond` (§3 space-members-in-prompt addition) so `assigned` can resolve to a real member id instead of free text. What's still deferred: wiring `githubRepo` (validated and stored, but otherwise inert) to actually sync approved tasks as GitHub issues.
